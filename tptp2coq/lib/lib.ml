...
| Cnf_anno af -> begin match af.af_formula with
...
| Cnf_anno af as t ->
  ...
  | R_negated_conjecture ->
    if is_equality t then true (* TODO: is this ok? *)
    else false
  | _ -> false
  end
| _ -> false
    ...
    List.concat (List.map (fun formula ->
      ...
      | Clause lits ->
        List.concat (List.filter_map (fun lit ->
          match lit with
          | Lit (_, a) -> Some (identifiers_with_rank_in_atom a) (* parse as Pos for Pos & Neg *)
          | _ ->
