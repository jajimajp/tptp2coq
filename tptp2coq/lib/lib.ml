...
let rec gen_coq_p_with_completion p use_lpo use_hammer use_smt use_twee =
  if use_hammer then gen_coq_p_with_completion_ham p
  else if use_smt then gen_coq_p_with_smt p
  else if use_twee then gen_coq_p_with_twee p
  else (
    (* prologue *)
    print_endline
      (String.concat "\n"
    ...
    (* HACK: coq-completion での証明の HACK のために、HintDB を作っておく *)
    (* 定数が一つもない場合には HintDB は作られない *)
    let rec aux = function
            | (n, 0) :: t -> Printf.printf "Hint Resolve %s.\n" n
            | _ :: t -> aux t
            | _ -> () in aux p.constants;
  ...
  let axiom_names =
    List.map
      (fun af ->
        match af.af_name with
        | N_word s -> Tptp_printer.show_plain_word s
        | _ -> failwith "gen_coq_p_with_twee: not implemented")
      p.axioms
  in
  let axiom_names = List.map (fun n -> "ax_" ^ n) axiom_names in
  (* TODO *)
  print_endline
    (String.concat "\n"
       [
         ...
         "Theorem " ^ goal_name ^ " : " ^ string_of_formula p problem_lit ^ ".";
         "Proof.";
         "  twee " ^ (String.concat " " axiom_names) ^ ".";
         "Qed.";
         "";
         "Check " ^ goal_name ^ ".";
         "";
